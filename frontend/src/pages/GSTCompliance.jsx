...
import { FileText, ShieldCheck, Calendar, Gift, AlertCircle, CheckCircle2, Loader2, ChevronDown, ChevronUp, RefreshCw } from 'lucide-react'
...
function TabBtn({ active, onClick, children }) {
  return (
    <button onClick={onClick}
      className={clsx('px-4 py-2 text-sm font-medium rounded-lg transition',
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={clsx('text-xs px-2 py-0.5 rounded-full border', styles[status] || 'bg-slate-600/50 text-slate-300 border-slate-500/30')}>
      {status}
    </span>
  )
}
...
function InvoicesTab() {
  ...
  const [form, setForm] = useState({ buyer_name: '', buyer_state: 'Maharashtra', buyer_gstin: '', product: 'Steel Rods', qty: 100, rate: 85 })

  const loadInvoices = async () => {
    setLoading(true); setError(null)
    try { setInvoices(await api.listInvoices()) }
  ...
  const generateInvoice = async () => {
    ...
    try {
      const result = await api.generateInvoice({
        ...
        line_items: [{ name: form.product, qty: Number(form.qty), unit: 'kg', rate: Number(form.rate) }],
      })
      await loadInvoices()
    } catch (e) {
  ...
  const validateReturn = async () => {
    ...
    try { setValidationResult(await api.validateReturn()) }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-card rounded-xl border border-border p-4">
        <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2"><FileText size={14} /> Generate GST Invoice</h3>
        ...
        <div className="flex gap-3 mt-4">
          <button onClick={generateInvoice} disabled={generating || !form.buyer_name.trim()}
          ...
          <button onClick={validateReturn} disabled={validating}
        ...
        {validationResult && (
          <div className={clsx('mt-3 p-3 rounded-lg border text-sm',
            validationResult.filing_ready ? 'bg-green-500/10 border-green-500/30' : 'bg-yellow-500/10 border-yellow-500/30')}>
            <div className="flex items-center gap-2 font-medium mb-2">
              ...
              <span className={validationResult.filing_ready ? 'text-green-400' : 'text-yellow-400'}>
                {validationResult.overall_status === 'valid' ? 'Return valid — ready to file' : 'Issues found — review before filing'}
              </span>
            </div>
            {validationResult.suggestions?.map((s, i) => <p key={i} className="text-xs text-slate-300 ml-5">✓ {s}</p>)}
            {validationResult.issues?.map((s, i) => <p key={i} className="text-xs text-red-300 ml-5">⚠ {s}</p>)}
          </div>
        )}
      </div>
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <span className="text-sm font-semibold text-white">All Invoices {!loading && `(${invoices.length})`}</span>
          <button onClick={loadInvoices} className="text-xs text-slate-400 hover:text-white flex items-center gap-1">
            <RefreshCw size={12} className={loading ? 'animate-spin' : ''} /> Refresh
          </button>
        </div>
        {loading ? (
          <div className="p-8 flex justify-center"><div className="w-6 h-6 border-2 border-brand border-t-transparent rounded-full animate-spin" /></div>
        ) : invoices.length === 0 ? (
          <div className="p-8 text-center text-slate-500 text-sm">No invoices found</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-border text-slate-400">
              ...
              <tbody>
                {invoices.map(inv => (
                  <tr key={inv.id} className="border-b border-border/50 hover:bg-white/5 transition">
                    ...
                    <td className="px-4 py-2.5 text-slate-300">₹{((inv.cgst || 0) + (inv.sgst || 0) + (inv.igst || 0)).toLocaleString('en-IN')}</td>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-card rounded-xl border border-border p-4">
        ...
        <button onClick={translateNotice} disabled={translating || !input.trim()}
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-xl border border-border p-8 text-center text-slate-500 text-sm">No GST notices yet</div>
      ) : (
              ...
              <div className="p-4 flex items-start justify-between cursor-pointer"
                ...
                <div className="flex-1 min-w-0 mr-3">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-medium text-white truncate">{notice.raw_text?.substring(0, 70)}...</span>
                    <StatusBadge status={notice.status} />
                  </div>
                  <p className="text-xs text-slate-500">{new Date(notice.created_at).toLocaleDateString('en-IN')}</p>
                </div>
                {expanded === notice.id ? <ChevronUp size={16} className="text-slate-400 shrink-0" /> : <ChevronDown size={16} className="text-slate-400 shrink-0" />}
              </div>
              {expanded === notice.id && (
                <div className="px-4 pb-4 space-y-3 border-t border-border pt-3">
                  ...
                  {notice.action_items?.length > 0 && (
                    <div>
                      ...
                      {notice.action_items.map((item, i) => (
                        <div key={i} className="flex items-start gap-2 text-sm text-slate-300 mb-1.5">
                          <span className="w-5 h-5 rounded-full bg-brand/20 text-brand text-xs flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                          {item}
                        </div>
                      ))}
                    </div>
...
function CalendarTab() {
  ...
  useEffect(() => {
    api.complianceCalendar('27AAPFU0939F1ZV')
      .then(setData)
      .catch(e => setError(e?.response?.data?.detail || e.message || 'Failed to load calendar'))
      .finally(() => setLoading(false))
  }, [])
    ...
    <div className="space-y-4">
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border text-slate-400 text-xs">
          ...
          <tbody>
            {data.deadlines?.map((d, i) => (
              <tr key={i} className="border-b border-border/50 hover:bg-white/5">
                ...
                <td className="px-4 py-2.5"><StatusBadge status={d.gstr1_status} /></td>
                <td className="px-4 py-2.5 text-slate-300 font-mono text-xs">{d.gstr3b_due}</td>
                <td className="px-4 py-2.5"><StatusBadge status={d.gstr3b_status} /></td>
          ...
          {schemes.map((s, i) => (
            <div key={i} className="bg-card rounded-xl border border-brand/30 p-4 hover:border-brand/60 transition">
              <div className="flex items-start justify-between mb-2">
                ...
                <span className="text-xs font-bold text-green-400 bg-green-500/20 px-2 py-0.5 rounded-full shrink-0 ml-2">{s.match_score}%</span>
              </div>
              <p className="text-xs text-brand mb-2">{s.benefit}</p>
              <p className="text-xs text-slate-400 mb-3">{s.match_reason}</p>
              <a href={`https://${s.apply_url}`} target="_blank" rel="noreferrer"
                className="text-xs text-brand hover:underline">Apply → {s.apply_url}</a>
            </div>
          ))}
        </div>
      )}
...
export default function GSTCompliance() {
  ...
  const TABS = [
    ...
    { id: 'notices', label: '⚠️ Notices', component: NoticesTab },
    { id: 'calendar', label: '📅 Filing Calendar', component: CalendarTab },
    { id: 'schemes', label: '🏛️ MSME Schemes', component: SchemesTab },
  ]
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      <div>
        ...
        <p className="text-sm text-slate-400">Invoice generation · Notice translation · Filing calendar · MSME schemes</p>
      </div>
