...
from database import get_db, Invoice, Buyer, HITLQueue, SellerProfile
from agents.gst_agent import generate_gst_invoice, validate_gstr_return
from utils.mock_gstn import verify_gstin, get_gstr1_summary, get_input_tax_credit
...
async def create_invoice(req: InvoiceRequest, db: Session = Depends(get_db)):
    ...
    invoice_data = await generate_gst_invoice(
...
async def validate_return(db: Session = Depends(get_db)):
    ...
    result = await validate_gstr_return(invoice_list)
    return result
...
async def lookup_hsn_code(product: str):
    import json, os
    hsn_path = os.path.join(os.path.dirname(__file__), "../utils/hsn_codes.json")
    with open(hsn_path) as f:
        hsn_db = json.load(f)
    product_lower = product.lower().replace(" ", "_")
    for key, val in hsn_db.items():
        if key in product_lower or any(w in product_lower for w in key.split("_")):
            return {"product": product, **val}
    return {"product": product, "hsn": "9999", "description": product, "gst_rate": 18}
