...
from database import get_db, GSTNotice, Invoice, NoticeType, MSMEScheme
from agents.compliance_agent import (
    translate_gst_notice,
    match_msme_schemes,
    generate_gstr_summary,
)
...
async def translate_notice(req: NoticeRequest, db: Session = Depends(get_db)):
    ...
    try:
        result = await translate_gst_notice(req.raw_text, notice_types_dict)
...
async def find_schemes(req: SchemeRequest, db: Session = Depends(get_db)):
    ...
    schemes = await match_msme_schemes(req.annual_turnover, req.employee_count, req.category, schemes_list)
...
async def gstr_summary(period: str, db: Session = Depends(get_db)):
    ...
    return await generate_gstr_summary(invoice_list, period)
