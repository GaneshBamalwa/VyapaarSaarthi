...
async def list_orders(month: Optional[str] = None, skip: int = 0, limit: int = 50, db: Session = Depends(get_db)):
    if not month:
        month = datetime.utcnow().strftime("%Y-%m")
    repo = OrderRepository(db)
    orders, total = repo.list_all(month=month, skip=skip, limit=limit)
...
async def emit_internal_event(body: EmitEventRequest):
    """Internal webhook for standalone processes (like Telegram Bot) to trigger WebSocket broadcasts in Uvicorn."""
    from app.websocket.manager import manager
    await manager.emit_order_event(body.order_id, body.status, body.data)
