...
def extract_phone_from_webhook(user_id: str) -> str | None:
    """
    The Meta Cloud API sends message.from as a raw phone number,
    e.g. "919876543210".  We normalise it to "+919876543210".
    Returns None if empty or clearly invalid.
    """
    if not user_id:
        return None
    cleaned = user_id.strip()
    # Strip whatsapp: prefix just in case (Twilio style)
...
async def handle_text_message(client: WhatsAppClient, user_id: str, text: str) -> None:
    ...
    phone = extract_phone_from_webhook(user_id)
