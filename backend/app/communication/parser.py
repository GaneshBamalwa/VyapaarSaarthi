...
class RegexIntentParser(IntentParser):
    def parse(self, text: str) -> IntentParseResult:
        ...
        # Generate Invoice
        if "invoice" in text or "bill" in text or text.startswith("/invoice"):
            match = re.search(r'(?i)(?:ord-|order\s*|number\s*)(\d+)', text)
            if match:
                entities["order_id"] = int(match.group(1))
            else:
                # crude customer extraction for invoice
                cust_match = re.search(r'(?i)(?:for|ka)\s+([a-zA-Z\s]+)', text)
                if cust_match:
                    # avoid matching keywords like "invoice" or "chahiye"
                    name = cust_match.group(1).replace("invoice", "").replace("chahiye", "").replace("dedo", "").strip()
                    if name:
                        entities["customer"] = name
            return IntentParseResult(intent=IntentType.GENERATE_INVOICE, entities=entities)

        # Download Excel / Report
        if "excel" in text or "sheet" in text or "xlsx" in text or "download" in text or "report" in text or text.startswith("/excel"):
            report_type = "both"
            if "order" in text and "expense" not in text:
                report_type = "orders"
            elif "expense" in text or "enpense" in text and "order" not in text:
                report_type = "expenses"
            return IntentParseResult(intent=IntentType.DOWNLOAD_EXCEL, entities={"report_type": report_type})
...
class GeminiIntentParser(IntentParser):
    def parse(self, text: str) -> IntentParseResult:
        ...
        # Fast bypass for explicit slash commands to prevent AI hallucination
        stripped = text.strip().lower()
        if stripped.startswith("/excel"):
            return IntentParseResult(intent=IntentType.DOWNLOAD_EXCEL, entities={"report_type": "both"})
        
        if stripped.startswith("/orders"): return IntentParseResult(intent=IntentType.LIST_ORDERS)
        if stripped.startswith("/pending"): return IntentParseResult(intent=IntentType.VIEW_PENDING)
        if stripped.startswith("/today"): return IntentParseResult(intent=IntentType.VIEW_TODAY)
        if stripped.startswith("/overdue"): return IntentParseResult(intent=IntentType.VIEW_OVERDUE)
...
If user asks for a report, excel, sheet, or invoice, map it to DOWNLOAD_EXCEL or GENERATE_INVOICE.
For DOWNLOAD_EXCEL, you MUST extract the 'report_type' entity (must be exactly "orders", "expenses", or "both" depending on what the user asked for).
